...
def create_local_venv_on_each_node(py_executable: str, venv_name: str):
    ...
    """
    # 检查是否跳过虚拟环境创建
    if os.environ.get("NRL_SKIP_VENV_CREATION", "false").lower() == "true":
        logger.info(f"Skipping venv creation for {venv_name} due to NRL_SKIP_VENV_CREATION=true")
        # 返回系统Python路径
        return sys.executable
    
    # 检查是否重用现有虚拟环境
    if os.environ.get("NRL_REUSE_EXISTING_VENV", "false").lower() == "true":
        # 尝试找到现有的虚拟环境
        NEMO_RL_VENV_DIR = os.path.normpath(
            os.environ.get("NEMO_RL_VENV_DIR", DEFAULT_VENV_DIR)
        )
